...
$sql = "SELECT * FROM it_2025_reg_tbl WHERE pay_status IN ('Paid', 'Complimentary', 'Free') AND srno < 50";
...
$category_map = [
    ...
    'Complimentary Delegate' => [1886, 'Delegate'],

    'Complimentary Exhibitors Delegate' => [1885, 'Delegate'],

    'Delegate' => [1886, 'Delegate'],

    'Full Delegate' => [1886, 'Delegate'],

    'GIA Partner' => [1877, 'GIA Partner'],

    'Media Delegate' => [1887, 'Media'],

    'Speaker' => [1869, 'Speaker'],
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $sub_delegate_count = (int)$row['sub_delegates']; // Get sub-delegates count

    $total_delegates += $sub_delegate_count;



    for ($i = 1; $i <= $sub_delegate_count; $i++) {

        $email = strtolower($row['email' . $i]);
        ...
        mysqli_stmt_bind_result($email_stmt, $email_count);
        ...
        if ($email_count > 0) {
            ...
            echo json_encode([
                ...
                'message' => "Email $email already registered. Skipping to next delegate.",
it_2025_
                'statuit_2025_kipped',
            ...
            continue; // Skip to the next delegate
        ...
        list($country_code, $mobile) = explode('-', $row['cellno' . $i]);

        

        // Assign category code and print value using array mapping

        $category_data = $category_map[$reg_cata] ?? [1886, 'Delegate'];

        list($category_id, $print) = $category_data;
        ...
        $insert_stmt = mysqli_prepare($db_connection, $sql);

        mysqli_stmt_bind_param($insert_stmt, 'sssisssssssss', 

            $name, $email, $reg_cata, $category_id, $country_code, $mobile, 

            $row['org'], $job_title, $print, $row['country'], $row['tin_no'], $row['city'], $row['pay_status']
        ...
        if (mysqli_stmt_execute($insert_stmt)) {
            ...
            echo "Inserted successfully for $email<br>\n";

        } else {

            echo "Error inserting data for $email: " . mysqli_error($db_connection) . "<br>\n";
        ...
        mysqli_stmt_close($insert_stmt); // Close statement after each insert
    ...
    echo "Total delegates processed: $total_delegates <br>\n";

    } else {

        echo "Error getting result: " . mysqli_error($db_connection);
