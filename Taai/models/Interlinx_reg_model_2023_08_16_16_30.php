...
class Interlinx_reg_model extends Base_Model {
    ...
    public function get_matched_data($criteria = array()) {
        ...
        if(isset($criteria['search-tej']) && !empty($criteria['search-tej'])) {
            ...
            $whereCondition = "( ( (fname LIKE '%" . $criteria['search'] . "%') OR (lname LIKE '%" . $criteria['search'] . "%') $cata ) AND (user_id != '" . $criteria['user_id'] . "') )";
            $order_by = '';
        }
        ...
        if(isset($criteria['search-tejind']) && !empty($criteria['search-tejind'])) {
            if(isset($criteria['catas'])) {
                ...
                $whereCondition = "(" . trim($like, 'OR') . ") AND (user_id != '" . $criteria['user_id'] . "')";
            } else {
                $whereCondition = "(" . $criteria['cata'] . " LIKE '%" . $criteria['search'] . "%') AND (user_id != '" . $criteria['user_id'] . "')";
            }
        }
